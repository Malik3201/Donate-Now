...
if($_SERVER['REQUEST_METHOD']==='POST'){
 ...
 if(!$errors && $ngoProfile && $ngoProfile['verification_status']==='verified'){
  $loc=location_coords_from_post();
  $stmt=$pdo->prepare('INSERT INTO campaigns (ngo_id, category_id, title, description, location_label, latitude, longitude, target_amount, image_url, imagekit_file_id, start_date, end_date, status) VALUES (:ngo_id,:category_id,:title,:description,:location_label,:latitude,:longitude,:target_amount,:image_url,:imagekit_file_id,:start_date,:end_date,:status)');
  $stmt->execute(['ngo_id'=>(int)$ngoProfile['id'],'category_id'=>null,'title'=>$title,'description'=>$description,'location_label'=>$loc['label'],'latitude'=>$loc['lat'],'longitude'=>$loc['lng'],'target_amount'=>$targetAmount,'image_url'=>$imageUrl,'imagekit_file_id'=>$imageFileId,'start_date'=>$startDate,'end_date'=>$endDate,'status'=>'pending']);
  ...
  foreach (admin_users_for_notifications($pdo) as $admin) {
    create_notification($pdo, (int) $admin['id'], 'New Campaign Submitted', 'A new campaign is waiting for admin review.', 'campaign');
  }
  $campaignAdminPayload = ['ngo_name' => (string) $ngoProfile['ngo_name'], 'title' => $title, 'target_amount' => (string) $targetAmount];
  foreach (admin_mail_recipients($pdo) as $adminEmail) {
    send_new_campaign_submitted_email($adminEmail, $campaignAdminPayload);
  }
...
<?php if ($ngoProfile && $ngoProfile['verification_status'] === 'verified'): ?>
<?= render_location_map_picker(null, null, null, null, '', true, false, true, 'dn-map-campaign-create') ?>
<?php endif; ?>
